...
from bin.train_evalute import evaluate, evaluate_save, train
from bin.Utils.load_datatsets import load_data
from bin.Utils.utils import get_device
...
def main(config, model_times, label_list):
  ...
  if config.do_train:
    ...
    train_dataloader, train_examples_len = load_data(
      config.task_list, config.data_dir, tokenizer, config.max_seq_length, config.train_batch_size, "train", label_list)
    dev_dataloader, _ = load_data(
      config.task_list, config.data_dir, tokenizer, config.max_seq_length, config.dev_batch_size, "dev", label_list)
    ...
    if config.model_name == "BertOrigin":
      from bin.BertOrigin.BertOrigin import BertOrigin
      ...
      from bin.FPNLU.FPNLU import FPNLU
